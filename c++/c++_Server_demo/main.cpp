...
static void *client_thread(void *arg) {
  ...
  try {
    while (true) {
      ...
      std::cout << "Client " << client_fd << " | Received: " << count
                << " bytes" << "str:" << std::string(buffer, count) << "\n";
    }
...
//
//       std::cout << " " << "clientfd: " << clientfd << " " << "count: " <<
//       count << "\n";
//     }
//   //getchar();
//   close(clientfd);
...
//   if (-1 == bind(sockfd, reinterpret_cast<const sockaddr *>(&serveraddr),
//   sizeof(sockaddr)))
//     {
//       perror("bind");
//       return -1;
...
//   int clientfd = accept(sockfd, reinterpret_cast<sockaddr *>(&clientaddr),
//   &len);
//
//   std::cout << "accept\n";
//
...
//   send(clientfd, buffer, count, 0);
//
//   std::cout << "sockfd: " << sockfd << " " << "clientfd: " << clientfd << " "
//   << "count: " << count << "\n";
//
// #else
//   //接收多次:
...
//       sockaddr_in clientaddr{};
//       socklen_t len = sizeof(clientaddr);
//       int clientfd = accept(sockfd, reinterpret_cast<sockaddr
//       *>(&clientaddr), &len); pthread_t thid; std::cout << "clientfd: " <<
//       clientfd << "\n"; pthread_create(&thid, nullptr, client_thread,
//       &clientfd);
//     }
//
// #endif
