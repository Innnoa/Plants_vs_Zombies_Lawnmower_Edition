...
fs::path CreateTempWorkspace() {
  std::string pattern =
      (fs::temp_directory_path() / "upgrade-refresh-resume-smoke-XXXXXX")
          .string();
...
void RunSmoke(const std::string& server_path) {
  ...
  login.set_player_name("upgrade_refresh_host");
  ...
  Require(login_result.success(), "upgrade refresh smoke: 登录失败");
  Require(login_result.player_id() > 0,
          "upgrade refresh smoke: player_id 非法");
  Require(!login_result.session_token().empty(),
          "upgrade refresh smoke: session_token 为空");

  lawnmower::C2S_CreateRoom create_room;
  create_room.set_room_name("upgrade_refresh_room");
  ...
  Require(create_result.success(), "upgrade refresh smoke: 建房失败");
  ...
  Require(game_start.success(), "upgrade refresh smoke: 开局失败");
  Require(game_start.room_id() == room_id,
          "upgrade refresh smoke: game_start room_id 不匹配");
  ...
  Require(upgrade_request_1.reason() == lawnmower::UPGRADE_REASON_LEVEL_UP,
          "upgrade refresh smoke: 首次升级原因不是 level_up");
  ...
  Require(options_1.options_size() >= 1,
          "upgrade refresh smoke: 第一轮升级选项为空");
  Require(options_1.refresh_remaining() == 1,
          "upgrade refresh smoke: 第一轮 refresh_remaining 不是 1");
  ...
  const auto upgrade_request_2 = ReceiveUpgradeRequestOrFailOnPrematureSnapshot(
      host, 2000, "refresh_upgrade_request");
  Require(upgrade_request_2.reason() == lawnmower::UPGRADE_REASON_REFRESH,
          "upgrade refresh smoke: refresh 后升级原因不是 refresh");
  Require(upgrade_request_2.player_id() == login_result.player_id(),
          "upgrade refresh smoke: refresh 后 player_id 不匹配");
  Require(upgrade_request_2.room_id() == room_id,
          "upgrade refresh smoke: refresh 后 room_id 不匹配");
  ...
  Require(options_2.options_size() >= 1,
          "upgrade refresh smoke: 刷新后升级选项为空");
  Require(options_2.refresh_remaining() == 0,
          "upgrade refresh smoke: 刷新后 refresh_remaining 不是 0");
  ...
  RequireChunkedFullSnapshot(
      host, "upgrade refresh smoke: refresh 后恢复即时全量快照", 300);
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cerr << "用法: upgrade_refresh_resume_smoke_test <server_binary>\n";
    return 2;
  }

  try {
    RunSmoke(argv[1]);
    std::cout << "upgrade_refresh_resume_smoke_test: PASS\n";
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "upgrade_refresh_resume_smoke_test: FAIL: " << ex.what()
              << "\n";
    return 1;
  }
}
