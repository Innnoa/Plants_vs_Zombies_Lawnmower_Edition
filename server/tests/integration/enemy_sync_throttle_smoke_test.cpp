...
fs::path CreateTempWorkspace() {
  std::string pattern =
      (fs::temp_directory_path() / "enemy-sync-throttle-XXXXXX").string();
...
void WriteTestConfigs(const fs::path& workspace, uint16_t tcp_port,
  ...
  WriteTextFile(cfg_dir / "server_config.json",
                ...
                "  \"tcp_port\": " +
                    ...
                    "  \"tick_rate\": 30,\n"
                    "  \"state_sync_rate\": 20,\n"
                    "  \"sync_enemy_near_distance\": 200,\n"
                    "  \"sync_enemy_far_distance\": 800,\n"
                    "  \"sync_enemy_medium_stride\": 2,\n"
                    "  \"sync_enemy_far_stride\": 5,\n"
                    "  \"map_width\": 4000,\n"
                    "  \"map_height\": 4000,\n"
                    "  \"enemy_spawn_base_per_second\": 1.2,\n"
                    "  \"enemy_spawn_per_player_per_second\": 0,\n"
                    "  \"enemy_spawn_wave_growth_per_second\": 0,\n"
                    "  \"max_enemies_alive\": 1,\n"
                    "  \"max_enemy_spawn_per_tick\": 1,\n"
                    "  \"max_enemy_replan_per_tick\": 8,\n"
...
void RunSmoke(const std::string& server_path) {
  ...
  login.set_player_name("enemy_sync_throttle_host");
  ...
  Require(login_result.success(), "enemy sync throttle smoke: 登录失败");
  Require(login_result.player_id() > 0,
          "enemy sync throttle smoke: player_id 非法");
  Require(!login_result.session_token().empty(),
          "enemy sync throttle smoke: session_token 为空");

  lawnmower::C2S_CreateRoom create_room;
  create_room.set_room_name("enemy_sync_throttle_room");
  ...
  Require(create_result.success(), "enemy sync throttle smoke: 建房失败");
  const uint32_t room_id = create_result.room_id();
  Require(room_id > 0, "enemy sync throttle smoke: room_id 非法");
  ...
  Require(game_start.success(), "enemy sync throttle smoke: 开局失败");
  ...
  std::unordered_map<uint32_t, std::vector<uint32_t>> enemy_ticks;
  bool saw_throttled_enemy = false;
  ...
  while (Clock::now() < deadline) {
    ...
    for (const auto& enemy : delta.enemies()) {
      auto& ticks = enemy_ticks[enemy.enemy_id()];
      if (!ticks.empty() && tick > ticks.back() && tick - ticks.back() >= 4) {
        saw_throttled_enemy = true;
        break;
      }
      ticks.push_back(tick);
    }
    if (saw_throttled_enemy) {
      break;
    }
  }

  Require(!enemy_ticks.empty(),
          "enemy sync throttle smoke: 未观察到任何敌人 delta");
  Require(saw_throttled_enemy,
          "enemy sync throttle smoke: 未观察到远距敌人的降频同步间隔");
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cerr << "用法: enemy_sync_throttle_smoke_test <server_binary>\n";
    return 2;
  }

  try {
    RunSmoke(argv[1]);
    std::cout << "enemy_sync_throttle_smoke_test: PASS\n";
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "enemy_sync_throttle_smoke_test: FAIL: " << ex.what() << "\n";
    return 1;
  }
}
