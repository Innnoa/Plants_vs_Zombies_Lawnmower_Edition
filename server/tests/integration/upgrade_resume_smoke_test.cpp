...
fs::path CreateTempWorkspace() {
  std::string pattern =
      (fs::temp_directory_path() / "upgrade-resume-smoke-XXXXXX").string();
...
void RunSmoke(const std::string& server_path) {
  ...
  login.set_player_name("upgrade_resume_host");
  ...
  Require(login_result.success(), "upgrade resume smoke: 登录失败");
  Require(login_result.player_id() > 0,
          "upgrade resume smoke: player_id 非法");
  Require(!login_result.session_token().empty(),
          "upgrade resume smoke: session_token 为空");

  lawnmower::C2S_CreateRoom create_room;
  create_room.set_room_name("upgrade_resume_room");
  ...
  Require(create_result.success(), "upgrade resume smoke: 建房失败");
  ...
  Require(game_start.success(), "upgrade resume smoke: 开局失败");
  Require(game_start.room_id() == room_id,
          "upgrade resume smoke: game_start room_id 不匹配");
  ...
  Require(upgrade_request.room_id() == room_id,
          "upgrade resume smoke: upgrade_request room_id 不匹配");
  Require(upgrade_request.player_id() == login_result.player_id(),
          "upgrade resume smoke: upgrade_request player_id 不匹配");
  Require(upgrade_request.reason() == lawnmower::UPGRADE_REASON_LEVEL_UP,
          "upgrade resume smoke: 升级原因不是 level_up");

  lawnmower::C2S_UpgradeRequestAck request_ack;
  request_ack.set_room_id(room_id);
  request_ack.set_player_id(login_result.player_id());
  ...
  Require(options.room_id() == room_id,
          "upgrade resume smoke: upgrade_options room_id 不匹配");
  Require(options.player_id() == login_result.player_id(),
          "upgrade resume smoke: upgrade_options player_id 不匹配");
  Require(options.options_size() >= 1,
          "upgrade resume smoke: 未收到任何升级选项");

  lawnmower::C2S_UpgradeOptionsAck options_ack;
  options_ack.set_room_id(room_id);
  options_ack.set_player_id(login_result.player_id());
  host.Send(lawnmower::MSG_C2S_UPGRADE_OPTIONS_ACK, options_ack);

  lawnmower::C2S_UpgradeSelect select;
  select.set_room_id(room_id);
  select.set_player_id(login_result.player_id());
  select.set_option_index(0);
  host.Send(lawnmower::MSG_C2S_UPGRADE_SELECT, select);

  const auto select_ack_packet =
      host.ReceiveUntil(lawnmower::MSG_S2C_UPGRADE_SELECT_ACK, 2000,
                        "upgrade_select_ack");
  const auto select_ack =
      ParsePayload<lawnmower::S2C_UpgradeSelectAck>(select_ack_packet);
  Require(select_ack.room_id() == room_id,
          "upgrade resume smoke: upgrade_select_ack room_id 不匹配");
  Require(select_ack.player_id() == login_result.player_id(),
          "upgrade resume smoke: upgrade_select_ack player_id 不匹配");
  Require(select_ack.option_index() == 0,
          "upgrade resume smoke: upgrade_select_ack option_index 不匹配");

  RequireChunkedFullSnapshot(host,
                             "upgrade resume smoke: 升级恢复后即时全量快照", 300);
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cerr << "用法: upgrade_resume_smoke_test <server_binary>\n";
    return 2;
  }

  try {
    RunSmoke(argv[1]);
    std::cout << "upgrade_resume_smoke_test: PASS\n";
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "upgrade_resume_smoke_test: FAIL: " << ex.what() << "\n";
    return 1;
  }
}
