...
fs::path CreateTempWorkspace() {
  std::string pattern =
      (fs::temp_directory_path() / "full-snapshot-chunking-XXXXXX").string();
 ...
 public:
  ServerProcess(std::string server_path, fs::path workspace)
    ...
    if (pid_ == 0) {
      ...
      ::execl(server_path.c_str(), server_path.c_str(), static_cast<char*>(nullptr));
 ...
 private:
  void WriteAll(const std::string& data) {
    ...
    while (offset < data.size()) {
      const ssize_t written = ::send(fd_, data.data() + offset, data.size() - offset, 0);
      if (written < 0) {
        if (errno == EINTR) {
          continue;
        }
...
void RequireChunkedFullSnapshot(TcpClient& client, const std::string& stage,
  ...
  const auto deadline = Clock::now() + std::chrono::milliseconds(timeout_ms);
  while (Clock::now() < deadline) {
    auto packet = client.ReceiveOnce(static_cast<int>(
        std::chrono::duration_cast<std::chrono::milliseconds>(deadline - Clock::now())
            .count()));
    if (!packet.has_value()) {
      continue;
    ...
    for (const auto& player : sync.players()) {
      Require(player.authoritative_tick() <= sync.sync_time().tick(),
              stage + ": player authoritative_tick 大于包 tick");
    }
    for (const auto& enemy : sync.enemies()) {
      Require(enemy.authoritative_tick() <= sync.sync_time().tick(),
              stage + ": enemy authoritative_tick 大于包 tick");
    }
    for (const auto& item : sync.items()) {
      Require(item.authoritative_tick() <= sync.sync_time().tick(),
              stage + ": item authoritative_tick 大于包 tick");
    }

    Require(sync.chunk_count() > 1,
            stage + ": 收到未分片的即时全量快照");
...
void RunSmoke(const std::string& server_path) {
  ...
  login.set_player_name("full_snapshot_chunk_host");
  host.Send(lawnmower::MSG_C2S_LOGIN, login);
  const auto login_packet = host.ReceiveUntil(lawnmower::MSG_S2C_LOGIN_RESULT, 3000);
  const auto login_result = ParsePayload<lawnmower::S2C_LoginResult>(login_packet);
  Require(login_result.success(), "full snapshot chunk smoke: 登录失败");

  lawnmower::C2S_CreateRoom create_room;
  create_room.set_room_name("full_snapshot_chunk_room");
  host.Send(lawnmower::MSG_C2S_CREATE_ROOM, create_room);
  const auto create_packet = host.ReceiveUntil(lawnmower::MSG_S2C_CREATE_ROOM_RESULT, 3000);
  const auto create_result = ParsePayload<lawnmower::S2C_CreateRoomResult>(create_packet);
  Require(create_result.success(), "full snapshot chunk smoke: 建房失败");
  ...
  const auto game_start_packet = host.ReceiveUntil(lawnmower::MSG_S2C_GAME_START, 3000);
  const auto game_start = ParsePayload<lawnmower::S2C_GameStart>(game_start_packet);
  Require(game_start.success(), "full snapshot chunk smoke: 开局失败");

  RequireChunkedFullSnapshot(host, "full snapshot chunk smoke: 开局即时全量", 400);
  ...
  const auto reconnect_ack = ParsePayload<lawnmower::S2C_ReconnectAck>(reconnect_packet);
  Require(reconnect_ack.success(), "full snapshot chunk smoke: 重连失败");
  RequireChunkedFullSnapshot(reconnect_client, "full snapshot chunk smoke: 重连即时全量",
                             400);
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cerr << "用法: full_snapshot_chunking_smoke_test <server_binary>\n";
    return 2;
  }

  try {
    RunSmoke(argv[1]);
    std::cout << "full_snapshot_chunking_smoke_test: PASS\n";
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "full_snapshot_chunking_smoke_test: FAIL: " << ex.what() << "\n";
    return 1;
  }
}
