...
fs::path CreateTempWorkspace() {
  std::string pattern =
      (fs::temp_directory_path() / "room-sync-budget-XXXXXX").string();
...
void WriteTestConfigs(const fs::path& workspace, uint16_t tcp_port,
  ...
  WriteTextFile(cfg_dir / "server_config.json",
                ...
                "  \"tcp_port\": " +
                    ...
                    "  \"tick_rate\": 30,\n"
                    "  \"state_sync_rate\": 20,\n"
                    "  \"sync_enemy_near_distance\": 100000,\n"
                    "  \"sync_enemy_far_distance\": 100000,\n"
                    "  \"sync_enemy_medium_stride\": 1,\n"
                    "  \"sync_enemy_far_stride\": 1,\n"
                    "  \"room_sync_object_budget_per_tick\": 6,\n"
                    "  \"room_event_entry_budget_per_tick\": 256,\n"
                    "  \"room_packet_budget_per_tick\": 8,\n"
                    "  \"map_width\": 2400,\n"
                    "  \"map_height\": 2400,\n"
...
class UdpClient final {
 public:
  UdpClient(std::string host, uint16_t port) {
    ...
    if (::bind(fd_, reinterpret_cast<const sockaddr*>(&local), sizeof(local)) !=
        0) {
      const int err = errno;
      ::close(fd_);
      fd_ = -1;
  ...
  void Send(lawnmower::MessageType type, const google::protobuf::Message& msg) {
    ...
    if (sent < 0 || static_cast<std::size_t>(sent) != data.size()) {
      Fail("UDP sendto 失败: " + std::string(std::strerror(errno)));
    }
  }
  ...
  std::optional<lawnmower::Packet> ReceiveOnce(int timeout_ms) {
    ...
    const ssize_t bytes = ::recv(fd_, buffer.data(), buffer.size(), 0);
    if (bytes < 0) {
      Fail("UDP recv 失败: " + std::string(std::strerror(errno)));
    }
    lawnmower::Packet packet;
    if (!packet.ParseFromArray(buffer.data(), static_cast<int>(bytes))) {
...
void RunSmoke(const std::string& server_path) {
  ...
  login.set_player_name("room_sync_budget_host");
  ...
  Require(login_result.success(), "room sync budget smoke: 登录失败");
  Require(login_result.player_id() > 0,
          "room sync budget smoke: player_id 非法");
  Require(!login_result.session_token().empty(),
          "room sync budget smoke: session_token 为空");

  lawnmower::C2S_CreateRoom create_room;
  create_room.set_room_name("room_sync_budget_room");
  ...
  Require(create_result.success(), "room sync budget smoke: 建房失败");
  const uint32_t room_id = create_result.room_id();
  Require(room_id > 0, "room sync budget smoke: room_id 非法");
  ...
  Require(game_start.success(), "room sync budget smoke: 开局失败");
  ...
  input.mutable_input_time()->set_tick(1);
  input.set_input_seq(1);
  ...
  std::unordered_map<uint32_t, uint32_t> per_tick_enemy_entries;
  std::set<uint32_t> unique_enemy_ids;
  uint32_t total_enemy_entries = 0;
  auto deadline = Clock::now() + std::chrono::seconds(3);
  while (Clock::now() < deadline) {
    auto packet = udp.ReceiveOnce(120);
    if (!packet.has_value()) {
      continue;
    }
    ...
    const uint32_t tick = delta.has_sync_time() ? delta.sync_time().tick() : 0;
    uint32_t& per_tick = per_tick_enemy_entries[tick];
    per_tick += static_cast<uint32_t>(delta.enemies_size());
    total_enemy_entries += static_cast<uint32_t>(delta.enemies_size());
    for (const auto& enemy : delta.enemies()) {
      unique_enemy_ids.insert(enemy.enemy_id());
    }
  }

  Require(!per_tick_enemy_entries.empty(),
          "room sync budget smoke: 未观察到任何敌人 delta");
  uint32_t max_enemy_entries_per_tick = 0;
  for (const auto& [_, count] : per_tick_enemy_entries) {
    max_enemy_entries_per_tick = std::max(max_enemy_entries_per_tick, count);
  }

  Require(max_enemy_entries_per_tick <= 6,
          "room sync budget smoke: 单 tick 敌人同步对象数超预算");
  Require(total_enemy_entries >= 12,
          "room sync budget smoke: 总敌人样本过少，未形成延后发送");
  Require(unique_enemy_ids.size() >= 8,
          "room sync budget smoke: 观察到的敌人实例过少");
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cerr << "用法: room_sync_budget_smoke_test <server_binary>\n";
    return 2;
  }

  try {
    RunSmoke(argv[1]);
    std::cout << "room_sync_budget_smoke_test: PASS\n";
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "room_sync_budget_smoke_test: FAIL: " << ex.what() << "\n";
    return 1;
  }
}
