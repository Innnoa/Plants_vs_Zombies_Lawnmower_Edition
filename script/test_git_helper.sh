...
pager_repo="$tmp_dir/pager"
git init "$pager_repo" >/dev/null
git -C "$pager_repo" config user.name "Pager Test"
git -C "$pager_repo" config user.email "pager@example.com"
install_helper "$pager_repo"
echo "pager seed" > "$pager_repo/PAGER.md"
git -C "$pager_repo" add PAGER.md script/git_helper.sh
git -C "$pager_repo" commit -m "chore: pager seed" >/dev/null
git -C "$pager_repo" branch pager-branch

pager_log_output="$(
  cd "$pager_repo" &&
  GIT_PAGER=missing-pager PAGER=missing-pager "$pager_repo/script/git_helper.sh" log
)"
assert_contains "$pager_log_output" "chore: pager seed"

pager_switch_output="$(
  printf '5\npager-branch\n8\n' | (
    cd "$pager_repo" &&
    GIT_PAGER=missing-pager PAGER=missing-pager "$pager_repo/script/git_helper.sh" menu
  ) 2>&1
)"
assert_contains "$pager_switch_output" "本地分支列表:"
assert_contains "$pager_switch_output" "退出菜单"
pager_current_branch="$(git -C "$pager_repo" symbolic-ref --short HEAD)"
assert_equals "$pager_current_branch" "pager-branch"

confirm_repo="$tmp_dir/confirm"
git init "$confirm_repo" >/dev/null
git -C "$confirm_repo" config user.name "Confirm Test"
git -C "$confirm_repo" config user.email "confirm@example.com"
install_helper "$confirm_repo"
echo "confirm seed" > "$confirm_repo/CONFIRM.md"
git -C "$confirm_repo" add CONFIRM.md script/git_helper.sh
git -C "$confirm_repo" commit -m "chore: confirm seed" >/dev/null

echo "pending line" >> "$confirm_repo/CONFIRM.md"
confirm_cancel_output="$(printf '4\ncancel commit\nno\n8\n' | run_helper_in_repo "$confirm_repo" menu 2>&1)"
assert_contains "$confirm_cancel_output" "以下改动将被暂存并提交："
assert_contains "$confirm_cancel_output" "已取消提交。"
cancel_head_message="$(git -C "$confirm_repo" log -1 --pretty=%s)"
assert_equals "$cancel_head_message" "chore: confirm seed"
if ! grep -q "pending line" "$confirm_repo/CONFIRM.md"; then
  fail "取消提交后工作区改动应保留"
fi

confirm_yes_output="$(printf '4\nconfirmed commit\nyes\n8\n' | run_helper_in_repo "$confirm_repo" menu 2>&1)"
assert_contains "$confirm_yes_output" "以下改动将被暂存并提交："
assert_contains "$confirm_yes_output" "退出菜单"
confirm_head_message="$(git -C "$confirm_repo" log -1 --pretty=%s)"
assert_equals "$confirm_head_message" "confirmed commit"
